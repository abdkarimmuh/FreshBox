...
class ProcedureUpdateSalesOrderDetail extends Migration
{
    ...
    public function up()
    {
        ...
        BEGIN
        UPDATE master_sales_order_detail SET name = name, sales_order_id = sales_order_id, skuid = skuid, uom_id = uom_id, name = name, qty = qty, amount_price = amount_price, tax_value = tax_value, total_amount = total_amount, notes = notes, status = status, updated_by = updated_by, updated_at = now() WHERE id = v_id;
        END');
    }
