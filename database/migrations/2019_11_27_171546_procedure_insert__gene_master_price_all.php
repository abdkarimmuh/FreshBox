...
class ProcedureInsertGeneMasterPriceAll extends Migration
{
    /**
     * Run the migrations.
     */
    public function up()
    {
        DB::unprepared('DROP PROCEDURE IF EXISTS GeneMasterPriceAll');

        DB::unprepared("CREATE PROCEDURE GeneMasterPriceAll( IN UserID INT)
        BEGIN
            START TRANSACTION;
                insert into master_price_log
                select a.*, UserID, now() from
                master_price as a,
                `price_temp` as b,
            ...
            -- 		and c.`customer_group_id` = customer_group
                    and a.`end_periode` < b.`Start_period`;
                ...
                , b.`updated_at` = now()
                ,b.`start_periode` = a.`start_period`
                , b.`end_periode` = a.`end_period`
                ...
                on
                    ...
                    `master_customer` as c
                on
                    c.`customer_group_id` = a.customer_group_id
                where
                    a.`SKU` not in
                    (select `skuid` from
                        `master_price` as mp,
                    ...
                    and mc.`customer_group_id` = a.customer_group_id )
                    and a.final > 0;
            COMMIT;
        END");
    }
