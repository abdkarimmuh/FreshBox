<template>
  <div>
    <div v-if="data.data && data.data.length > 1">
      ...
      ></slot>
      <data-table-filters v-else @getData="getData" :per-page="perPage" :tableData="tableData"></data-table-filters>
      <vue-table
        @sort="sortBy"
        :sortKey="sortKey"
        :columns="columns"
      ...
      >
        <tbody :class="classes['t-body']">
          <tr :key="item.id" :class="classes['t-body-tr']" v-for="item in data.data">
            <td :key="column.name" v-for="column in columns" :class="classes.td">
              <data-table-cell
                :value="item"
                :name="column.name"
                :meta="column.meta"
    ...
    <div v-if="data.data && !data.data.length" class="text-center p-3 text-muted">
      <h5>No Results</h5>
      <p>Looks like you have not added any data yet!</p>
    </div>

    <slot name="pagination" v-if="paginationSlot" :links="data.links" :meta="data.meta"></slot>
    <laravel-pagination
      v-else
      :data="data"
...
import VueTable from "./partials/Table.vue";
import DataTableCell from "./partials/DataTableCell.vue";
import DataTableFilters from "./partials/DataTableFilters.vue";
